...
| Unsupported transport closes without destroying runner/tmux state | Implemented; pending CI | `test_unsupported_transport_closes_without_disturbing_runner_or_tmux` |
...
| Panel remount while offline preserves state and reattaches after recovery | Implemented; pending CI | `test_preserves_runner_offline_state_across_a_panel_remount_and_reattaches_after_recovery` |
...
| Unsupported transport performs one control-to-PTY fallback and does not loop | Implemented; pending CI | `test_unsupported_transport_closes_without_disturbing_runner_or_tmux`; `falls back from control to PTY once and stops on a second unsupported close` |
...
| `terminal-e2e-reconnect` | Re-running | Runner generation, stale lifecycle, approval reconnect, unsupported transport, and browser remount recovery |
