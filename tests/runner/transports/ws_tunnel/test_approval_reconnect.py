...
async def test_pending_local_action_executes_once_after_runner_tunnel_reconnect(
    ...
    runner_app.state.local_action_gateway._publish_audit = lambda record: audits.append(
        record.to_event()
    )
    workspace = runner_app.state.local_action_gateway._workspaces.advertise(home=tmp_path)[0]
